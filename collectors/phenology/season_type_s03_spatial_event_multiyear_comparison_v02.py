#!/usr/bin/env python3
"""
SeedTrade.eu — LT S03 2021–2023 spatial-event comparison v0.2

Fix vs v0.1:
- Do NOT require equal target-crop pixel counts across years.
  CTY crop coverage can legitimately change by year.
- Instead verify that the underlying S03 raster geometry is identical across years:
  CRS, bbox, resolution, width, height.
- Preserve same conflict/event comparison logic.
...
def find_raw_s03(year):
    exact = DATA / (
        f"LT_VALIDATION_multiyear_validation_pixel_intelligence_"
        f"{year}_S03_2km_400ha_test_2026-09-07.json"
    )
    if exact.exists():
        return exact

    matches = sorted(DATA.glob(
        f"LT_VALIDATION_multiyear_validation_pixel_intelligence_{year}_S03_2km_400ha_test_*.json"
    ))
    if len(matches) == 1:
        return matches[0]
    if not matches:
        raise FileNotFoundError(f"No LT raw S03 file found for {year}")
    raise RuntimeError(
        f"Ambiguous LT raw S03 files for {year}: {[x.name for x in matches]}"
    )
...
def raw_geometry(year):
    path = find_raw_s03(year)
    ...
    g = {
        ...
        "width_pixels": d.get("width_pixels"),
        "height_pixels": d.get("height_pixels"),
        "sample_center": d.get("sample_center"),
    }
    return g
...
def analyse_year(year):
    ...
    pixels = data["samples"]["S03"]["pixels"]
    ...
    ranked = sorted(by_sig.items(), key=lambda kv: len(kv[1]), reverse=True)
    ...
    return {
        ...
        "all_conflicts_bbox": bbox_rc(conflict_coords),
...
def main():
    geometries = {y: raw_geometry(y) for y in YEARS}

    geometry_keys = ("crs", "bbox", "resolution_m", "width_pixels", "height_pixels")
    geometry_equal = all(
        all(geometries[y][k] == geometries[YEARS[0]][k] for k in geometry_keys)
        for y in YEARS[1:]
    )
    ...
    integrity = {
        "same_underlying_s03_raster_geometry": geometry_equal,
        ...
        "note": (
            "Target-crop pixel counts are allowed to differ by year because "
            "the yearly CTY crop classification can change."
        ),
    }
    integrity["pass"] = (
        integrity["same_underlying_s03_raster_geometry"]
        and integrity["2023_s03_conflicts_pass"]
    )
    ...
    out = DATA / (
        f"LT_VALIDATION_S03_spatial_event_multiyear_comparison_v02_"
    ...
    print("=" * 116)
    print("LT S03 2021–2023 SPATIAL-EVENT COMPARISON v0.2")
    ...
    print(f"Same underlying raster geometry: {geometry_equal}")
    print("Target-crop pixel counts may differ by year.")
    print()

    print("RAW S03 GEOMETRY")
    for y in YEARS:
        g = geometries[y]
        print(
            f"  {y}: CRS={g['crs']} | res={g['resolution_m']}m | "
            f"{g['width_pixels']}x{g['height_pixels']} | bbox={g['bbox']}"
        )
    print()
