"""
SeedTrade.eu
Copernicus Pixel Intelligence v0.8 CPCSY validation
...
8. CPCSY    = Number of growing seasons detected in the year (0/1/2)

Current purpose:
Add CPCSY as an eighth, independent validation layer to the validated 7-layer crop-season evidence workflow in one 2 x 2 km (400 ha) test area. CPCSY is preserved as evidence only and does NOT change winter/spring classification.

IMPORTANT:
- This v0.8 run is a 400 ha CPCSY validation test; winter/spring classification remains a separate downstream step.
- CPCSY is NOT a winter/spring classifier. It reports the number of detected growing seasons in the calendar year.
...
# CPCSY is kept local in v0.8 so the existing copernicus_layers.py
# registry and the validated v0.6/v0.7 pipeline do not need to change.
CPCSY_COLLECTION_ID = "93db885a-db11-4a7d-b035-bd6020a4447d"
CPCSY_BAND = "CPCSY"
CPCSY_DATA_TYPE = f"byoc-{CPCSY_COLLECTION_ID}"


def layer_data_type(layer_code):
    if layer_code == "CPCSY":
        return CPCSY_DATA_TYPE
    return get_data_type(layer_code)


def layer_collection_id(layer_code):
    if layer_code == "CPCSY":
        return CPCSY_COLLECTION_ID
    return get_collection_id(layer_code)


def layer_band(layer_code):
    if layer_code == "CPCSY":
        return CPCSY_BAND
    return get_band(layer_code)
...
def request_access_token(
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent":
                "SeedTrade.eu-Pixel-Intelligence/0.8",
        },
    )
...
def save_tiff(
    ...
    path = (
        OUTPUT_DIR
        / (
            f"PL_EAST_pixel_intelligence_v08_cpcsy_"
            f"{TEST_YEAR}_"
            f"2km_400ha_test_"
            f"{current_date}.tif"
        )
...
def interpret_cpcsy(value):
    ...
    flag = special_quality_flag(value)
...
def print_result(
    ...
    print(
        "SeedTrade.eu Copernicus Pixel Intelligence v0.8 CPCSY validation"
    )
    ...
    print(
        "CPCSY is evidence only and is NOT used to decide "
        "WINTER_CYCLE_SIGNAL or SPRING_CYCLE_SIGNAL."
    )
...
def main():
    ...
    try:
        ...
        print(
            "Requesting 8-layer Copernicus Data Fusion with CPCSY validation..."
        )
        ...
        result = {
            ...
            "cpcsy_validation": {
                ...
                "meaning":
                    "Number of growing seasons detected within the calendar year",
