...
VALIDATION_MODE = "OUT_OF_SAMPLE_FROZEN_YEAR"
...
def find_v04():
    return find_latest(
        "*season_type_classifier_v04_multiyear_validation_"
...
def main():
    ...
    print(f"PL_EAST | {VALIDATION_YEAR} | frozen out-of-sample validation")
    ...
    result = {
        ...
        "scientific_note":
            (
                f"This is an out-of-sample frozen-rule validation for {VALIDATION_YEAR}. "
                "v0.7 uses the published CLMS Zone 1 seasonal geometry "
                "as the primary cycle label and retains SeedTrade v0.4 "
                "as a crop-specific validation layer. Internal confidence "
                "tiers describe agreement between evidence layers and are "
                "not official CLMS confidence values. CPCSY is a separate "
                "CLMS cross-layer signal and does not identify winter "
                "versus spring."
            ),
    }
    ...
    print(f"Out-of-sample validation year: {VALIDATION_YEAR}")
    print()
    print("JSON saved to:")
    print(output_path)
